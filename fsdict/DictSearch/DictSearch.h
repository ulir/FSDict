#ifndef FSDICT_DICTSEARCH_H
#define FSDICT_DICTSEARCH_H FSDICT_DICTSEARCH_H
...
#include<fsdict/LevFilter/LevFilter.h>
...
namespace fsdict {
     ...
     *
     * fsdict::DictSearch is a combined interface for a number of modules of the package. The scenario is that the users
	...
	/**
	 * @brief This is a specialisation of fsdict::Interpretation, tailored to store additional
	 ...
	 */
	class iResultReceiver : public fsdict::LevFilter::CandidateReceiver,
				public fsdict::iInterpretationReceiver {
	...
	private:
	    ...
	    std::vector< fsdict::DictSearch::Interpretation > myVector_;
	...
	public:
		...
		if( ( p < 0 ) || ( p > 100 ) ) throw fsdict::exceptions::LogicalError( "fsdict::DictSearch::DictModule::setPriority: value ranges from 0 to 100" );
		priority_ = p;
	    }
	...
	public:
		...
		if( dLev > 3 ) throw exceptions::LogicalError( "fsdict::DictSearch::ConfigLookup::setDLevStatic: choose a threshold in the range [0..3]" );
	...
	public:
	     ...
	     * Note that all options belong to the namespace fsdict::Global.
	     *
	     */
	 ...
	 * @param patternFile A file containing a list of rewrite patterns. Consult fsdict::PatternSet for details
	 *
...
namespace std {
    inline wostream& operator<<( wostream& os, fsdict::DictSearch::Interpretation const& obj ) {
	obj.print( os );
	return os;
    }
}
