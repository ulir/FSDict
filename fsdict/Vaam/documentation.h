...
- which dictionary to use as base. It is obligatory to pass a reference to a MinDic-object in the constructor (fsdict::Vaam::Vaam).
- which set of patterns to use. It is obligatory to specify a path to a set of patterns in the constructor (fsdict::Vaam::Vaam).
...
The method fsdict::Vaam:query asks for two arguments: the query token and a pointer to an answer object. The latter must
be of the type std::vector< Interpretation >* . This is where Vaam will store the possible interpretations of the query
string. The example program fsdict/Vaam/example.cxx demonstrates how to interpret the structure of an Interpretation object.
