#ifndef FSDICT_TRANSTABLE_STANDARD_TCC
#define FSDICT_TRANSTABLE_STANDARD_TCC FSDICT_TRANSTABLE_STANDARD_TCC
...
namespace fsdict {
	...
	if( susoStrings_ == 0 ) {
	    throw exceptions::LogicalError( "fsdict::TransTable_standard::getSusoString: called while not in construction phase" );
	}
...
//	    std::cerr << "ffc hack: " << firstFreeCell_ << std::endl;
	...
	if( elementsRead != 1 ) {
	    throw exceptions::badInput( "fsdict::TransTable< TT_STANDARD >::loadFromStream: could not read Header" );
	}
	...
	if( elementsRead != nrOfCells_ ) {
	    throw exceptions::badInput( "fsdict::TransTable< TT_STANDARD >::loadFromStream: could not read cells." );
	}
