#ifndef FSDICT_TRANSTABLE_PERFHASH_TCC
#define FSDICT_TRANSTABLE_PERFHASH_TCC FSDICT_TRANSTABLE_PERFHASH_TCC
...
//	    std::cerr << "ffc hack: " << firstFreeCell_ << std::endl;
	...
	if( fread( &header_, sizeof( Header ), 1, fi ) != 1 ) {
	  throw exceptions::badDictFile( "fsdict::TransTable::loadFromStream: Read error while reading header" );
	}
	...
	if( fread( cells_, sizeof( Cell_t ), nrOfCells_, fi ) != nrOfCells_ ) {
	  throw exceptions::badDictFile( "fsdict::TransTable::loadFromStream: Read error while reading transTable" );
	}
	...
	if( fread( susoStrings_, sizeof( wchar_t ), lengthOfSusoStrings_, fi ) != lengthOfSusoStrings_ ) {
	  throw exceptions::badDictFile( "fsdict::TransTable::loadFromStream: Read error while reading suso strings" );
	}
