namespace fsdict {
	...
	inline void MinDic2::compileDic( const char* txtFile ) {
		...
		while( std::getline( fileHandle, line ).good() )  {
			...
			if ( line.length() > Global::lengthOfLongStr ) {
				throw exceptions::badInput( "fsdict::MinDic2::compileDic: Maximum length of input line violated (set by Global::lengthOfLongStr)" );
			}
	...
	inline void MinDic2::addToken( const wchar_t* key, const AnnType_t& annotation ) {
		...
		if( lengthOfKey == 0 ) {
			throw exceptions::badInput( "fsdict::MinDic2: Insertion of empty key " );
		}
	...
	inline void MinDic2::printDic() const {
	    std::wcout << "fsdict::MinDic2::printDic not implemented!" << std::endl;
	    exit( 1 );
		count_ = 0;
