#ifndef FSDICT_GETOPT_H
#define FSDICT_GETOPT_H FSDICT_GETOPT_H
...
// 	}
	...
	for( size_t i = 1; i < argc; ++i ) {
		...
		if( pos != std::string::npos ) {
		    ...
		    throw Exception( "fsdict::Getopt::getOptions: invalid argument structure" );
		}
	    }
	    else { // is no flag
		addArgument( word );
	...
	for( size_t i = 1; i < argc; ++i ) {
		...
		if( ! keyWaitsForValue.empty() ) {
		    std::string msg = "fsdict::Getopt::getOptionsAsSpecified: expected value after non-void option ";
		...
		if( pos != std::string::npos ) { // '=' inside the string
			...
			std::string msg = "fsdict::Getopt::getOptionsAsSpecified: no value expected for void option ";
			msg += key;
			throw Exception( msg );
		    }
		    optionValues_[ key ] = value;
	...
	if( ! keyWaitsForValue.empty() ) {
	    std::string msg = "fsdict::Getopt::getOptionsAsSpecified: expected value for non-void option ";
	    msg += keyWaitsForValue;
	    throw Exception( msg );
	}

	if( ! obligatory_.empty() ) {
	    std::string msg = std::string( "fsdict::Getopt::getOptionsAsSpecified: obligatory options still missing: " );
...
// 	}
	...
	if( ! hasOption( key ) ) throw std::runtime_error( "fsdict::Getopt::getOption: no such key defined" );
	...
	if( idx >= getArgumentCount() ) throw std::runtime_error( "fsdict::Getopt::getArgument: out of range." );
