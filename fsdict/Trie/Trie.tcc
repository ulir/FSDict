namespace fsdict {
	...
	inline void Trie< AnnType_t >::compileDic( const char* txtFile ) {
		...
		while( std::getline( fileHandle, line ).good() )  {
			...
			if ( line.length() > Global::lengthOfLongStr ) {
				throw exceptions::badInput( "fsdict::Trie::compileDic: Maximum length of input line violated (set by Global::lengthOfLongStr)" );
			}
	...
	inline void Trie< AnnType_t >::addToken( const wchar_t* key, const AnnType_t& annotation ) {
		...
		if( lengthOfKey == 0 ) {
			throw exceptions::badInput( "fsdict::Trie: Insertion of empty key " );
		}
	...
	inline bool Trie< AnnType_t >::getAnnotation( const wchar_t* key, AnnType_t* annotation ) const {
		std::cerr<<"fsdict::Trie::getAnnotation: This method is deprecated. Instead, use method lookup() with the same signature"<<std::endl;
	...
	inline const AnnType_t& Trie< AnnType_t >::annotationsAt( size_t n ) const {
		if( n >= nrOfKeys_ ) throw exceptions::outOfRange( "fsdict::Trie: out-of-range request for annotation-array." );
