...
namespace fsdict {
	...
	if ( !fi ) {
	    throw exceptions::badFileHandle( "fsdict::FBDicString: Couldn't open file '" +
					     std::string( dicFile ) +
					     "' for reading." );
	}
	loadFromStream( fi );
	...
	while( fileHandle.getline(( char* ) bytesIn, Global::lengthOfLongStr ) )  {
		...
		*c = 0;
	    ...
	    else throw exceptions::badInput( "fsdict::FBDicString::compileDic: No string annotation given." );

	    if( mbstowcs( key, (const char*)bytesIn, Global::lengthOfLongStr ) == (size_t)-1 ) {
		throw exceptions::badInput( "fsdict::FBDicString::compileDic: Invalid utf-8 sequence" );
	    }

	    if( ! ( c  && *key ) ) {
		throw exceptions::badInput( "fsdict::FBDicString::compileDic: wrong input format" );
	    }
