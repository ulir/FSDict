...
namespace fsdict {
    ...
    void ComputeInstruction::connectPatternProbabilities( fsdict::PatternProbabilities const& patternProbabilities ){
	...
	if( wCorr.size() >= 45 || wErr.size() >= 45 ) {
	    throw exceptions::badInput( "fsdict::ComputeInstruction; input words too long" );
	}
	
 	//std::wcerr << "compare " << wCorr << ", " << wErr << std::endl;

	if( ! patternProbabilities_ ) {
	    throw exceptions::LogicalError( "fsdict::ComputeInstruction::ComputeInstruction: No patternProbabilities loaded." );
	    exit( 1 );
	}
    ...
    /**
     * @brief recursive method to find all best paths through the levenshtein-matrix and store the resulting fsdict::Instruction s
     */
