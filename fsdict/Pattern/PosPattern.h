#ifndef FSDICT_POSPATTERN
#define FSDICT_POSPATTERN FSDICT_POSPATTERN
...
namespace fsdict {
	...
	if( str.at( leftBracket ) != '(' ) throw exceptions::badInput( "fsdict::PosPattern::parseFromString: Found no opening bracket" );
	offset += 1;

	size_t leftRightDelimiter = str.find( Pattern::leftRightDelimiter_, offset );
	if( leftRightDelimiter == str.npos ) throw exceptions::badInput( "fsdict::Pattern::parseFromString: Found no leftRightDelimiter" );

	offset = leftRightDelimiter  + 1;
	size_t comma = str.find( ',', offset );
	if( comma == str.npos ) throw exceptions::badInput( "fsdict::Pattern::parseFromString: Found no comma" );

	offset = comma  + 1;
	size_t rightBracket = str.find( ')', offset );
	if( rightBracket == str.npos ) throw exceptions::badInput( "fsdict::Pattern::parseFromString: Found no closing bracket" );
...
namespace std {
    inline wostream& operator<<( wostream& os, fsdict::PosPattern const& obj ) {
	obj.print( os );
	return os;
    }
}
