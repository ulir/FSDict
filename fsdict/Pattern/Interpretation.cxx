#ifndef FSDICT_INTERPRETATION_CXX
#define FSDICT_INTERPRETATION_CXX FSDICT_INTERPRETATION_CXX
...
namespace fsdict {
	...
	if( colon == str.npos ) throw exceptions::badInput( "fsdict::Interpretation::parseFromString: Found no colon" );
	setWord( str.substr( offset, colon - offset ) );

	offset = colon + 1;
	size_t plus = str.find( '+', offset );
	if( plus == str.npos ) throw exceptions::badInput( "fsdict::Interpretation::parseFromString: Found no plus" );
	...
	if( str.substr( offset, 6 ) != L",dist=" ) throw exceptions::badInput( "fsdict::Interpretation::parseFromString: Found no ',dist='" );
	offset += 6;
	...
	size_t begin = 0;
	for( fsdict::Instruction::const_iterator posPattern = instruction_.begin(); posPattern != instruction_.end(); ++posPattern ) {
