...
namespace fsdict {
    ...
    void PatternWeights::setWeight( const fsdict::Pattern& pattern, double weight ) {
	if( weight == UNDEF ) {
	...
	if( errno == EILSEQ ) { // catch encoding error
	    throw exceptions::badInput( "fsdict::PatternWeights: Encoding error in input sequence." );
	}
	...
	for( std::vector< std::pair< fsdict::Pattern, double > >::const_iterator it = histPatternCountSorted.begin();
