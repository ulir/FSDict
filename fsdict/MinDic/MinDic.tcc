namespace fsdict {
	...
	inline void MinDic< AnnType_t >::loadFromStream( FILE* fi ) {
	  ...
	  if( fread( &header_, sizeof( Header ), 1, fi ) != 1 ) {
	    throw exceptions::badDictFile( "fsdict::MinDic::loadFromStream: Read error while reading header" );
	  }
		...
		if( fread( annotations_, sizeof( AnnType_t ), header_.getNrOfKeys(), fi ) != header_.getNrOfKeys() ) {
		  throw exceptions::badDictFile( "fsdict::MinDic::loadFromStream: Read error while reading annotations" );
		}
	}
	...
	inline const AnnType_t& MinDic< AnnType_t >::annotationsAt( size_t n ) const {
		if( n >= nrOfKeys_ ) throw exceptions::outOfRange( "fsdict::MinDic: out-of-range request for annotation-array." );
