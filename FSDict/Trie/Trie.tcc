namespace csl {
	...
	inline Trie< AnnType_t >::Trie( const char* dicFile ) {
		if( dicFile ) {
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::loadFromFile( const char* binFile ) {
		FILE* fi = fopen( binFile, "rb" );

		if( ! fi ) {
			throw exceptions::badFileHandle( std::string( "Trie: Couldn't open file '" ) +
				std::string( binFile ) +
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::loadFromStream( FILE* fi ) {
		// read the Trie-Header
		fread( &header_, sizeof( Header ), 1, fi );

		if( header_.getMagicNumber() != magicNumber_ ) {
			throw exceptions::badDictFile( "Trie: Magic number comparison failed.\n" );
		}
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::writeToFile( char const* binFile ) const {
		FILE* fo = fopen( binFile, "wb" );
		if( ! fo ) {
			throw exceptions::badFileHandle( "Trie: Couldn't open file '" +
				std::string( binFile ) +
				"' for writing." );
		}
		writeToStream( fo );
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::writeToStream( FILE* fo ) const {
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::initConstruction() {
	    // std::wcerr<<"Trie::initConstruction()"<<std::endl;
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::finishConstruction() {
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::compileDic( const char* txtFile ) {
		wchar_t* key = 0;
		...
		while( std::getline( fileHandle, line ).good() )  {
			...
			if ( line.length() > Global::lengthOfLongStr ) {
				throw exceptions::badInput( "csl::Trie::compileDic: Maximum length of input line violated (set by Global::lengthOfLongStr)" );
			}
		...
		if( errno == EILSEQ ) { // catch encoding error
		    throw exceptions::badInput( "Trie::compileDic: Encoding error in input sequence." );
		}
	...
	template<>
	inline void Trie< int >::parseAnnotation( std::wstring* str, AnnType_t* annotation ) const {
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::addToken( const wchar_t* key, const AnnType_t& annotation ) {
		...
		if( lengthOfKey == 0 ) {
			throw exceptions::badInput( "csl::Trie: Insertion of empty key " );
		}

		if( lengthOfKey > Global::lengthOfStr - 1 ) {
			throw exceptions::badInput( "Trie: Global::string_length sets the maximum string length for an entry of a Trie. Maximum violated" );
		}
		...
		if( *lastKey_ && ( wcscmp( lastKey_, key ) >= 0 ) ) {
			std::wcerr<<"Alphabetical order violated:"<<std::endl
				...
				<<key<<std::endl
				<<"**********"<<std::endl;
			throw exceptions::badInput( "Trie::addToken: Alphabetical order violated" );
			exit( 1 );
		}
		...
		if( sizeOfAnnotationBuffer_ <= ( nrOfKeys_ + 1 ) ) {
		    ...
		    if( !annotations_ ) throw( exceptions::cslException( "csl::Trie: could not re-allocate memory for annotation array." ) );
		}
	...
	template< class AnnType_t >
	inline bool Trie< AnnType_t >::lookup( const wchar_t* key, AnnType_t* annotation ) const {
		size_t tokIdx = 0;
	...
	template< class AnnType_t >
	inline bool Trie< AnnType_t >::lookup( std::wstring const& key, AnnType_t* annotation ) const {
	    return lookup( key.c_str(), annotation );
	}
	
	template< class AnnType_t >
	inline bool Trie< AnnType_t >::hasPrefix( std::wstring const& prefix ) const {
	...
	template< class AnnType_t >
	inline bool Trie< AnnType_t >::getAnnotation( const wchar_t* key, AnnType_t* annotation ) const {
		std::cerr<<"csl::Trie::getAnnotation: This method is deprecated. Instead, use method lookup() with the same signature"<<std::endl;
		return lookup( key, annotation );
	}

	template< class AnnType_t >
	inline const AnnType_t& Trie< AnnType_t >::getAnnotation( size_t perfHashValue ) const {
		return annotationsAt( perfHashValue );
	}

	template< class AnnType_t >
	inline const AnnType_t& Trie< AnnType_t >::annotationsAt( size_t n ) const {
		if( n >= nrOfKeys_ ) throw exceptions::outOfRange( "csl::Trie: out-of-range request for annotation-array." );
		return annotations_[n];
	}

	template< class AnnType_t >
	inline size_t Trie< AnnType_t >::getNrOfKeys() const {
		return nrOfKeys_;
	}

	template< class AnnType_t >
	inline void Trie< AnnType_t >::printDic() const {
		count_ = 0;
...
// 	}
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::printDic_rec( StateId_t pos, int depth, size_t perfHashValue ) const {
		int newPos;
	...
	template< class AnnType_t >
	inline void Trie< AnnType_t >::doAnalysis() const {
